...
public class TargetSentimentCompiler extends LinearNetworkCompiler {
	...
	LinearNetwork[] unlabelnetwork = new LinearNetwork[5000];
	LinearNetwork[] labelnetwork = new LinearNetwork[5000];
	...
	void visualize(LinearNetwork network, String title, int networkId)
	{
		...
		LinearInstance t = (LinearInstance)network.getInstance();
		int id = t.getInstanceId();
		if (Math.abs(id) > 0)//357 
		{
			if (id < 0)
			{
				unlabelnetwork[-id] = network;
				
				if (labelnetwork != null)
				{
					//viewer.visualizeNetwork(unlabelnetwork[-id], null, "unlabel" + "[" + networkId + "]");
					//viewer.visualizeNetwork(labelnetwork[-id], null, "label" + "[" + networkId + "]" );
					System.out.println("Visualize inst id:" + t.getInstanceId());
					
					
					boolean contains = unlabelnetwork[-id].contains(labelnetwork[-id]);
					System.err.println(-id + "\t[un]contains?" + contains);
			...
			{
				labelnetwork[id] = network;
				
				//viewer.visualizeNetwork(labelnetwork[id], null, "label" + "[" + networkId + "]");
				
				/*
				if (unlabelnetwork != null)
				{
					System.out.println("contains?" + unlabelnetwork.contains(labelnetwork));
				}*/
