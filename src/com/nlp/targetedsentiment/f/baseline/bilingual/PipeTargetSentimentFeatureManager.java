package com.nlp.targetedsentiment.f.baseline.bilingual;
...
import com.nlp.targetedsentiment.f.baseline.bilingual.PipeTargetSentimentCompiler.*;
...
public class PipeTargetSentimentFeatureManager extends LinearFeatureManager {
	...
	public FeatureArray extract_helper(Network network, int parent_k,
		...
		if (ids_parent[4] == NodeType.Span.ordinal()) //!input_token[pos_parent].getName().startsWith("O")
		{
			...
			feature.add(new String[]{FEATURE_TYPES.Bigram + "", subnode_parent + "",  subnode_child + ""});
		...
		if (feature.size() > 0)
		{
			if (TargetSentimentGlobal.ENABLE_WORD_EMBEDDING)
			{
				...
				{
					String word = WordEmbeddingFeature + "_" + inst.lang;
					vector = TargetSentimentGlobal.Word2Vec.getVector(word);
					
				}
				
				int normal_feature_size = feature.size();
				int f[] = new int[normal_feature_size + ((vector == null) ? 0 : TargetSentimentGlobal.Word2Vec.ShapeSize)];
				...
				if (vector != null)
				{
					...
					{
						f[normal_feature_size + i] = this._param_g.toFeature(network, TargetSentimentGlobal.Word2Vec.WORD_EMBEDDING_FEATURE, Out, "" + i);					
