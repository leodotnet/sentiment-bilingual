		...
		for(int j = 0;j<network.countNodes(); j++){
			...
			for(int k = start; k<this.countNodes() ; k++){
				...
				if(node1==node2){
					...
					if (children1.length == 1 && children1[0].length == 0)
					{
					...
					for(int[] child1 : children1){
						...
						if(!child_found){
							...
							for(long node : child1_nodes)
							{
								int[] node_ids = NetworkIDMapper.toHybridNodeArray(node);
								System.err.print(Arrays.toString(node_ids) + ", ");
							}
							System.err.println();
							for(int t = 0; t<children2.length; t++){
								...
								long[] hypernodes = this.toNodes(children2[t]);
								for(long node : hypernodes)
								{
									int[] node_ids = NetworkIDMapper.toHybridNodeArray(node);
									System.err.print(Arrays.toString(node_ids) + ", ");
								}
							...
							System.err.println("does not contain!");//throw new RuntimeException("does not contain!");
							return false;
						}
					}
					}
